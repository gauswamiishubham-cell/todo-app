...
// ── In-memory "database" (persists during session via module-level vars) ──
...
const css = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .auth-wrap {
    ...
    padding: 24px;
    background: radial-gradient(ellipse at 60% 0%, #1e1a3a 0%, ${C.bg} 60%);
  }
  .auth-card {
    width: 100%;
    max-width: 420px;
  ...
  .pill.low  { color: ${C.success}; }
  .pill.med  { color: ${C.warn}; }
  .pill.high { color: ${C.danger}; }
      ...
      <div className="auth-card">
        ...
        {tab === "signup" && (
          <div className="field">
            ...
            <input value={name} onChange={e => setName(e.target.value)} onKeyDown={handleKey} placeholder="Jane Doe" />
          </div>
        )}
        <div className="field">
          <label>Email</label>
        ...
        <div className="field">
          ...
          <input type="password" value={pass} onChange={e => setPass(e.target.value)} onKeyDown={handleKey} placeholder={tab === "signup" ? "At least 6 characters" : "••••••••"} />
        </div>
...
function Dashboard({ user, onLogout }) {
  ...
  const save = (updated) => {
    db.tasks[user.id] = updated;
    setTasks([...updated]);
  };

  const addTask = () => {
    if (!input.trim()) return;
    const task = { id: uid(), title: input.trim(), priority, done: false, created: Date.now() };
    save([task, ...tasks]);
    setInput("");
  };
    ...
    <div className="dash">
      ...
      <main className="main">
        ...
        <div className="stats">
          ...
          <div className="stat-card"><div className="stat-num" style={{ color: C.success }}>{done}</div><div className="stat-lbl">Completed</div></div>
        </div>
        ...
        <div className="add-task-box">
          ...
          <div className="priority-row">
            ...
            {["low", "med", "high"].map(p => (
              <button
                key={p}
                className={`pill ${p}${priority === p ? " active" : ""}`}
        ...
        <div className="task-list">
          {filtered.length === 0 ? (
            ...
            <div key={task.id} className={`task-card${task.done ? " done" : ""}`}>
              ...
              <button className="delete-btn" onClick={() => remove(task.id)} title="Delete task">×</button>
            </div>
          ))}
        </div>
      </main>
